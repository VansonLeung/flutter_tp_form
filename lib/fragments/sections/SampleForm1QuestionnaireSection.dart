...
class SampleForm1QuestionnaireSectionState extends State<SampleForm1QuestionnaireSection> with SingleTickerProviderStateMixin {
  ...
  dynamic doSummarizeQuestionnaire() {
    var dict = {};
    dict["q1"] = fse_q1.tvAny.value.join(",");
    dict["q2"] = fse_q2.tvAny.value.join(",");
    dict["q3"] = fse_q3.tvAny.value.join(",");
    dict["q4"] = fse_q4.tvAny.value.join(",");
    dict["q5"] = fse_q5.tvAny.value.join(",");
    return dict;
  }
    ...
    return Column(
      children: <Widget>[
        ...
        ...formItemList1.asMap().entries.map((entry) {
          return entry.value.renderFormItemListByIndex(formItemList1, entry.key);
        }).toList(),
